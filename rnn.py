...
class Model(object):
    ...
    def initialize_model(self):
        ...
        self.x = tf.placeholder(tf.float32, shape = (None, wv_dim, self.num_steps))
        self.y = tf.placeholder(tf.int32, shape = (None, self.num_steps))
        self.loan_amounts = tf.placeholder(tf.float32, shape = (None, self.num_steps))
        '''
        temp = tf.nn.embedding_lookup(wv, self.x)
        if self.num_steps > 1:
            inputs = tf.split(1, self.num_steps, temp)
            inputs = map(tf.squeeze, inputs)
        else:
            inputs = [tf.squeeze(temp)]
        inputs = map(lambda x: tf.cast(x, tf.float32), inputs)
        #print len(inputs)
        #inputs = tf.cast(temp, tf.float32)
        '''
        if self.num_steps > 1:
        ...
        self.initial_state_h1 = tf.zeros((self.batch_size, filter_number_1))
        self.initial_state_h2 = tf.zeros((self.batch_size, filter_number_2))
        h1 = self.initial_state_h1
        h2 = self.initial_state_h2
        rnn_outputs = []
        for idx, batch in enumerate(inputs):
            with tf.variable_scope("RNN") as scope:
                ...
                wc1 = tf.get_variable("wc1", (wv_dim, filter_number_1), 
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                bc1 = tf.get_variable("bc1", (filter_number_1,),
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                H1 = tf.get_variable("H1", (filter_number_1, filter_number_1),
                    initializer = tf.constant_initializer(value=0.0, dtype=tf.float32))
                print batch
                ''''''
                h1 = tf.nn.relu(tf.matmul(batch, wc1) + tf.matmul(h1, H1) + bc1)
                h1 = tf.nn.dropout(h1, self.keep_prob)
                wc2 = tf.get_variable("wc2", (filter_number_1, filter_number_2), 
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                bc2 = tf.get_variable("bc2", (filter_number_2,),
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                '''
                pred = tf.matmul(tf.concat(1, [loans[idx], h1]), wc2) + bc2 
                '''
                
                H2 = tf.get_variable("H2", (filter_number_2, filter_number_2),
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                h2 = tf.nn.relu(tf.matmul(h1, wc2) + tf.matmul(h2, H2) + bc2)
                wc3 = tf.get_variable("wc3", (filter_number_2 + 1, self.n_classes), 
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                bc3 = tf.get_variable("bc3", (self.n_classes,),
                    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))
                
                pred = bc3 + tf.matmul(tf.concat(1, [loans[idx], h2]), wc3)
                #theta = tf.get_variable("theta", (1, self.n_classes),
                #    initializer = tf.random_normal_initializer(mean=0.0, stddev=sigma, seed=None, dtype=tf.float32))               
                #pred = bc3 + tf.matmul(loans[idx], theta)
                self.previous_state_h1 = h1
                self.previous_state_h2 = h2
    ...
    def train(self, batch_xs, batch_ys, batch_loans, sess):
        #print batch_ys
        state1 = self.initial_state_h1.eval()
        state2 = self.initial_state_h2.eval()

        l, state1, state2, _ = sess.run([self.loss, self.previous_state_h1, self.previous_state_h2, self.optimizer], 
            feed_dict={self.x: batch_xs, self.y: batch_ys, self.keep_prob: self.dropout, 
            self.initial_state_h1: state1, self.initial_state_h2: state2, self.loan_amounts: batch_loans})
        return l
    ...
    def test(self, num_steps, batch_xs, batch_ys, batch_loans, sess):
        ...
        for i in range(num_steps):
            ...
            l, outputs, state1, state2 = sess.run([self.loss, self.output, self.previous_state_h1, self.previous_state_h2], feed_dict={self.x: data_in, self.y: label_in, 
                self.loan_amounts: batch_loans, self.initial_state_h1: state1, self.keep_prob: self.dropout})
...
with tf.variable_scope('RNNLM') as scope:
    ...
    with tf.Session() as sess:
        ...
        batch_xs, batch_ys, batch_loans = test_set.fetch_rnn_test(1000)
        acc = 0
        y_square = 0
        total_description_length = 0
        for idx in range(len(batch_xs)):
            ...
            #print batch_xs[idx].shape
            #print batch_ys[idx].shape
            acc += test_model.test(num_steps, 
                np.expand_dims(batch_xs[idx], axis = 0), np.expand_dims(batch_ys[idx], axis = 0), 
                np.expand_dims(batch_loans[idx], axis = 0), sess)
            #acc += sess.run(loss, feed_dict={x: np.reshape(batch_xs[idx], (1, 1, -1))
            #    , y: np.reshape(batch_ys[idx], (1, 1, -1)), keep_prob: 1.})
            y_square += np.sum(batch_ys[idx]**2)
        #acc = np.sqrt(acc / y_square)
        #print acc.shape
        acc /= total_description_length
        print 'final test error: ', np.sqrt(acc) * test_set.labels_std




        saver.restore(sess, 'rnn_nnet.weights')
